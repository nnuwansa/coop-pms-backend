...
from db.models.models import SystemUser, RefreshToken, LetterAssignee, Role, RolePermission, Permission
...
# NEW — used by the Initials By / Order By pickers on the Letter View page:
# only users whose ROLE currently grants the given permission code should
# show up as selectable, so the list narrows itself automatically as
# permissions are assigned/revoked in Manage Permissions, without a
# separate manual "who can be picked here" list to keep in sync.
async def get_system_users_by_permission_code(permission_code: str, db: Session):
    ...
    return (
        ...
        .filter(Permission.code == permission_code, SystemUser.is_active)
        .order_by(SystemUser.first_name, SystemUser.last_name)
        .distinct()
        .all()
    )
