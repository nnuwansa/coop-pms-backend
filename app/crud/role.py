...
async def get_role_status_ids(role_id: int, db: Session):
    rows = db.query(RoleStatusPermission.status_id).filter(
        RoleStatusPermission.role_id == role_id
    ).all()
    return [r[0] for r in rows]


async def update_role_status_permissions(role_id: int, status_ids: list[int], db: Session):
    db.query(RoleStatusPermission).filter(RoleStatusPermission.role_id == role_id).delete()
    db.add_all([RoleStatusPermission(role_id=role_id, status_id=sid) for sid in status_ids])
    db.commit()
