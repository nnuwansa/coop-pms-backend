...
# from db.models.models import SystemUser, Role, Department, Designation, DepartmentUnit
...
#         department_unit_id=user_model.department_unit_id,  # NEW
#     )
#
...
#         changes.append(
#             f"first name changed from '{_display(user_db.first_name)}' to '{_display(user_model.first_name)}'")
...
#         changes.append(
#             f"employee ID changed from '{_display(user_db.employee_id)}' to '{_display(user_model.employee_id)}'")
...
#         changes.append(
#             f"designation changed from '{_display(user_db.designation)}' to '{_display(user_model.designation)}'")
...
#     new_unit_id = user_model.department_unit_id if user_model.department_unit_id else None
#     if user_db.department_unit_id != new_unit_id:
#         old_unit_name = user_db.department_unit.name if user_db.department_unit else "None"
#         new_unit = db.query(DepartmentUnit).filter(DepartmentUnit.id == new_unit_id).first() if new_unit_id else None
#         new_unit_name = new_unit.name if new_unit else "None"
#         changes.append(f"sub-unit changed from '{old_unit_name}' to '{new_unit_name}'")
#
...
#     user_db.department_unit_id = new_unit_id
#     if user_model.password:
...
#         department_unit = db.query(DepartmentUnit).filter(
#             DepartmentUnit.id == payload.department_unit_id
#         ).first()
#         if not department_unit:
#             raise NoDataFoundException(f"Department unit with ID {payload.department_unit_id} not found")
#         department_id = department_unit.department_id
#
#     department = db.query(Department).filter(Department.id == department_id, Department.is_active).first()
#     if not department:
...
#         email=payload.email,
#         password=hash_pwd,
#         first_name=department_unit.name if department_unit else department.name,
#         last_name="",
#         department_id=department.id,
#         department_unit_id=department_unit.id if department_unit else None,
#         role_id=None,
#         is_active=True,
...
#             department_id=user.department_id,  # NEW
...
#
#
# async def get_department_accounts_service(db: Session):
#     logger.info("Get department accounts process started")
#
#     users = await get_department_accounts(db)
#     result = [
...
#             "department_name": user.department.name if user.department else None,
#             "department_unit_id": user.department_unit_id,                                   # NEW
#             "department_unit_name": user.department_unit.name if user.department_unit else None,  # NEW
#             "email": user.email,
#         }
#         for user in users
#     ]
...
# NEW — used by the Initials By picker on the Letter View page. Only users
# whose role currently grants the given permission code should be pickable,
# so the list stays correct automatically as roles are edited, rather than
# needing a separate manual list to keep in sync.
async def get_system_users_by_permission_service(permission_code: str, db: Session):
    from crud.system_user import get_system_users_by_permission_code
    from models.system_user import SystemUserByPermissionOut

    users = await get_system_users_by_permission_code(permission_code, db)
    return [
        SystemUserByPermissionOut(
            id=user.id,
            name=f"{user.first_name} {user.last_name}",
            is_default=bool(user.is_default_initials_by),
        )
        for user in users
    ]
...
async def update_system_user_info(
    ...
    # NEW — at most one user should ever have this flag set (it's what the
    # Initials By picker pre-selects by default). Rather than requiring the
    # admin to manually un-toggle the previous default, setting it true here
    # is treated as exclusive: it always wins.
    if user_model.is_default_initials_by is not None:
        if user_model.is_default_initials_by and not user_db.is_default_initials_by:
            db.query(SystemUser).filter(SystemUser.id != user_id).update(
                {SystemUser.is_default_initials_by: False}, synchronize_session=False
            )
            changes.append("set as the default Initials By person")
        elif not user_model.is_default_initials_by and user_db.is_default_initials_by:
            changes.append("removed as the default Initials By person")
        user_db.is_default_initials_by = user_model.is_default_initials_by
