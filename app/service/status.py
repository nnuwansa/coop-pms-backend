...
async def create_status(status_model: StatusModelIn, db: Session):
    ...
    status = Status(
        ...
        requires_file_name=status_model.requires_file_name,
        requires_copies_forwarded_to=status_model.requires_copies_forwarded_to,   # NEW
        requires_summary=status_model.requires_summary,                            # NEW
    )
