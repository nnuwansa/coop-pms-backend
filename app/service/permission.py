...
async def create_permission(permission_model: PermissionModelIn, db: Session):
    ...
    permission = Permission(
        ...
        code=permission_model.code,
        description=permission_model.description,
        category=permission_model.category,   # NEW — required NOT NULL column
        action=permission_model.action,        # NEW — required NOT NULL column
    )

    permission_db = await save_permission(permission, db)
    # CHANGED — a single created permission is a PermissionModel (id, name,
    # code, description), not a PermissionModelOut. PermissionModelOut is
    # shaped for the grouped list endpoint (category + action + a list of
    # permissions) and validating one row against it would fail — there is
    # no `permissions` list on a single Permission row.
    permission_response = PermissionModel(
        id=permission_db.id,
        name=permission_db.name,
        code=permission_db.code,
        description=permission_db.description,
    )
...
async def get_permission(permission_id: int, db: Session):
    ...
    # CHANGED — same reasoning as create_permission above.
    permission_response = PermissionModel(
        id=permission.id,
        name=permission.name,
        code=permission.code,
