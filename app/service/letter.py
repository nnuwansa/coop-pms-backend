...
# #         raise NoDataFoundException(f"Letter with ID {letter_id} not found")
# #
# #     # CHANGED — read directly from current_user (from JWT), no extra DB call
...
# #         allowed_department_ids=None,  # NEW
# #         allowed_assignee_role_ids=None,  # NEW
...
# #         if new_status:
# #             # CHANGED — generalized: any status flagged requires_file_name, not just "Completed"
...
# #         if allowed_department_ids:  # NEW — restrict which departments this role may route letters to
# #             disallowed = set(department_ids) - set(allowed_department_ids)
# #             if disallowed:
...
# #         # ── Assignees ─────────────────────────────────────────────────────────────
# #         if can_assign:
# #             if allowed_assignee_role_ids:  # NEW — restrict assignees to specific roles
# #                 assignee_role_ids = {
...
#         # ── Departments ───────────────────────────────────────────────────────────
# 
#     # FIXED — previously this whole department block only ran when
#     # `allowed_department_ids` happened to be truthy, so a user with no
#     # department restriction (the normal case) could never actually change
#     # departments even though `can_change_department` was True. The
#     # restriction check now only *validates*; the update itself always runs.
#     if can_change_department:
#         if allowed_department_ids:  # NEW — restrict which departments this role may route letters to
#             disallowed = set(department_ids) - set(allowed_department_ids)
#             if disallowed:
...
# 
#     # ── Assignees ─────────────────────────────────────────────────────────────
#     # FIXED — this block used to be nested inside `if can_change_department:`,
#     # which meant a user who could change status/assignee but did NOT have
#     # letter.change_department permission would have their assignee changes
#     # silently discarded (and, worse, nothing at all would be committed — see
#     # the db.commit() note below). It now runs based on `can_assign` alone,
#     # independent of the department permission.
#     if can_assign:
#         if allowed_assignee_role_ids:  # NEW — restrict assignees to specific roles
#             assignee_role_ids = {
#                 u.role_id for u in db.query(SystemUser).filter(SystemUser.id.in_(assignee_ids)).all()
...
#                 db.add(LetterAssignee(letter_id=letter_id, assignee_id=assignee_id))
# 
#     # FIXED — db.commit() used to live inside `if can_change_department:`,
#     # meaning that for any user without letter.change_department permission,
#     # NOTHING from this function was ever persisted — not the status change,
#     # not the assignee change, nothing. This is why selecting "Recommendation"
#     # status + an assignee appeared to silently fail to save and the letter
#     # stayed as if nothing happened. The commit now always runs once, after
#     # whichever of the three sections above actually applied.
#     db.commit()
#     logger.info("Update letter assignment process ended")
# 
# 
...
async def get_letter_by_id(
    ...
    letter_response = LetterModelOutOne(
        ...
        # NEW — who this letter was recommended to, kept entirely separate
        # from `assignees` so it never overwrites or gets overwritten by the
        # actual assignee list. Requires a `recommended_to` relationship
        # (FK: recommended_to_id -> system_user.id) on the Letter model.
        recommended_to=(
            IdNameModelOut(
...
async def get_list_letters(
    ...
    letters_response = [
        LetterModelOutList(
            ...
            other=letter.other,
            sender_subject_no=letter.sender_subject_no,
            completion_file_name=letter.completion_file_name,  # NEW
        )
        for letter in letters_db
    ]
...
async def letters_excel(filters: LetterExcelFilter, current_user: SystemUserWithPermissionsModelOut, db: Session):
    ...
    # NEW — organizational heading at the top of the report. `print_title_rows`
    # below makes these rows (plus the header row) repeat automatically on
    # every printed page when the sheet is printed from Excel, so the report
    # stays identifiable no matter how many pages it spans.
    ...
    # NEW — repeat the heading + header rows (1 through 4) at the top of
    # every page when this workbook is printed.
...
async def update_letter_assignment(
        ...
        can_assign: bool = True,
        file_name: Optional[str] = None,  # NEW
        recommended_to_id: Optional[int] = None,  # NEW
):
    logger.info("Update letter assignment process started")
    ...
    # ── Recommended To ───────────────────────────────────────────────────────
    # NEW — deliberately separate from the Assignees block above. Previously
    # "who to recommend this to" was just stuffed into the same assignee list,
    # so picking a recommendation target replaced (removed) the person the
    # letter was actually assigned to. Now the two are independent: assignees
    # keeps whoever is actually working the letter, and recommended_to_id is
    # purely an additional pointer to who the Recommendation should go to.
    # Reuses `can_assign` since recommending is a form of directing the letter.
...
async def update_cheque_deposit(letter_id: int, payload: "ChequeDepositIn", db: Session, current_user) -> None:
    ...
    db.add(HistoryModel(
        description=f"C"
                    f"heque {'marked as deposited' if payload.deposited else 'deposit unmarked'}",
        username=f"{current_user.first_name} {current_user.last_name}",
        email=current_user.email,
        letter_id=letter_id,
    ))
