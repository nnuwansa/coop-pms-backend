...
async def quick_add_designation_endpoint(
        designation: DesignationModelIn,
        db: DbSession,
        _=Depends(has_permission("letter.order_by"))
):
    # NEW - lets whoever holds letter.order_by add a missing seal/designation
    # title inline from the Order By picker, without needing the broader
    # user.view permission that the main create endpoint above requires.
    logger.debug(f'Request to quick-add designation from Order By picker: {designation}')
    designation_response = await create_designation(designation, db)
    return GenericResponse(success=True, message="Designation created successfully", data=designation_response)