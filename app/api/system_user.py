...
#
# from models.system_user import DepartmentAccountIn
# from service.system_user import create_department_account as _create_department_account
# from service.system_user import create_system_user, \
#     get_all_system_user_full_names_service, update_system_user_info, delete_system_user_info, get_list_users, \
#     get_system_user_stats, get_user_history_service, get_department_accounts_service   #
#
...
#
#
# @router.post("/department-account", status_code=HTTP_201_CREATED, response_model=GenericResponse)
# async def create_department_account_endpoint(
#         payload: DepartmentAccountIn,
#         db: DbSession,
#         current_user=Depends(has_permission("user.view")),
# ):
#     logger.debug(f'Request to create department account: {payload.department_id}')
#     result = await _create_department_account(payload, db, performed_by=getattr(current_user, "email", None))
#     return GenericResponse(success=True, message="Department account created successfully", data=result)
#
#
#
#
# @router.get("/department-accounts", response_model=GenericResponse)
# async def get_department_accounts_endpoint(
#         db: DbSession,
#         _=Depends(get_current_user)
# ):
#     logger.debug('Request to fetch department accounts')
#     result = await get_department_accounts_service(db)
#     return GenericResponse(success=True, message="Department accounts fetched successfully", data=result)
...
@router.get("/by-permission/{permission_code}", response_model=GenericResponse)
async def get_system_users_by_permission_endpoint(
        permission_code: str,
        db: DbSession,
        _=Depends(get_current_user)
):
    # NEW — used by the Letter View's Initials By picker: only users whose
    # role currently grants `permission_code` should be selectable there.
    logger.debug(f'Request to fetch system users with permission: {permission_code}')
    from service.system_user import get_system_users_by_permission_service
    users = await get_system_users_by_permission_service(permission_code, db)

    return GenericResponse(success=True, message="System users fetched successfully", data=users)
